...
app.put('/make-server-25b11ac0/pedidos/:id', async (c) => {
  try {
    ...
    console.log(
      '   Estado asignaciones:',
      data.asignaciones?.map((a) => ({ num: a.camareroNumero, estado: a.estado }))
    );
...
app.get('/make-server-25b11ac0/informes/camarero', async (c) => {
  try {
    ...
    for (const pedido of pedidos) {
      const matchFecha =
        (!desde || pedido.diaEvento >= desde) && (!hasta || pedido.diaEvento <= hasta);
...
app.get('/make-server-25b11ac0/confirmar/:token', async (c) => {
  try {
    ...
    console.log(
      `✅ CONFIRMACIÓN: Camarero ${camarero?.nombre} ${camarero?.apellido} confirmó asistencia al evento "${pedido.cliente}"`
    );
    ...
    const todosConfirmados =
      asignaciones.length > 0 && asignaciones.every((a) => a.estado === 'confirmado');
    ...
    if (todosConfirmados) {
      ...
      if (!chatExistente) {
        ...
        console.log(
          `✅ Chat grupal creado automáticamente para pedido: ${pedido.cliente} (Expira: ${fechaEliminacion.toISOString()})`
        );
      }
    }
...
app.get('/make-server-25b11ac0/no-confirmar/:token', async (c) => {
  try {
    ...
    if (pedido) {
      ...
      console.log(
        `❌ RECHAZO: Camarero ${camarero?.nombre} ${camarero?.apellido} rechazó el evento "${pedido.cliente}"`
      );
      console.log(`   Estado actualizado: rechazado`);
      console.log(
        `   Eliminación programada: ${new Date(Date.now() + 5 * 60 * 60 * 1000).toISOString()}`
      );
  ...
  try {
    ...
    const diagnostico = {
      ...
      resumen: {
        ...
        // NUEVO
        eventosCompletosSinChat: infoEventos.filter((e) => e.todosConfirmados && !e.tieneChat)
          .length,
      ...
      try {
        ...
        if (!coordinadorId) {
          resultados.push({
            ...
            success: false,
            error:
              'No se puede determinar coordinadorId (no está en el pedido ni se proporcionó uno por defecto)',
          });
          continue;
        }
...
app.get('/make-server-25b11ac0/chats/:coordinadorId', async (c) => {
  try {
    ...
    if (todosLosChats.length > 0) {
      console.log(
        '🔍 IDs de coordinadores en todos los chats:',
        todosLosChats.map((c) => ({ chatId: c.id, coordinadorId: c.coordinadorId }))
      );
    }
    ...
    for (const chat of chatsDelCoordinador) {
      ...
      if (ahora < fechaExpiracion) {
        ...
        console.log(
          `🗑️ Chat eliminado por expiración: ${chat.id} - Expiró el ${fechaExpiracion.toISOString()}`
        );
      }
    }

    console.log(
      `📊 Chats activos para coordinador ${coordinadorId}: ${chatsActivos.length} de ${chatsDelCoordinador.length}`
    );
...
async function enviarEmailGenerico({
  destinatario,
  cc,
  asunto,
  htmlBody,
  ...
  console.log(
    `  RESEND_API_KEY: ${resendApiKey ? `configurada (${resendApiKey.length} chars)` : 'NO CONFIGURADA'}`
  );
  console.log(
    `  SENDGRID_API_KEY: ${sendgridApiKey ? `configurada (${sendgridApiKey.length} chars)` : 'NO CONFIGURADA'}`
  );
  console.log(
    `  MAILGUN_API_KEY: ${mailgunApiKey ? `configurada (${mailgunApiKey.length} chars)` : 'NO CONFIGURADA'}`
  );
  ...
  return {
    success: false,
    error:
      'No hay ningún servicio de email configurado o todos fallaron. Por favor, configura RESEND_API_KEY, SENDGRID_API_KEY, o MAILGUN_API_KEY en las variables de entorno.',
  };
}
...
app.get('/make-server-25b11ac0/verificar-email-config', async (c) => {
  try {
    ...
    console.log(
      `  RESEND_API_KEY: ${resendApiKey ? `✓ configurada (${resendApiKey.length} chars, inicia con: ${resendApiKey.substring(0, 5)}...)` : '✗ NO CONFIGURADA'}`
    );
    console.log(
      `  SENDGRID_API_KEY: ${sendgridApiKey ? `✓ configurada (${sendgridApiKey.length} chars)` : '✗ NO CONFIGURADA'}`
    );
    ...
    console.log(
      `  MAILGUN_DOMAIN: ${mailgunDomain ? `✓ configurado: ${mailgunDomain}` : '✗ NO CONFIGURADO'}`
    );
    console.log(`  EMAIL_FROM: ${emailFrom}`);
...
app.post('/make-server-25b11ac0/enviar-email-parte', async (c) => {
  try {
    ...
    if (pdfBase64) {
      ...
      console.log(
        `✅ PDF generado exitosamente: ${nombreArchivo} (${Math.round(pdfBase64.length / 1024)} KB)`
      );
    } else {
  ...
  try {
    ...
    if (!whatsappApiKey || !whatsappPhoneId) {
      return c.json({
        ...
        message:
          'WhatsApp Business API no está configurado. Necesitas configurar WHATSAPP_API_KEY y WHATSAPP_PHONE_ID en las variables de entorno.',
    ...
    if (whatsappApiKey.length < 100) {
      return c.json({
        ...
        message:
          '⚠️ ERROR: El WHATSAPP_API_KEY es demasiado corto. Un token válido debe tener más de 200 caracteres. Es posible que hayas usado el Phone ID como token.',
    ...
    if (whatsappApiKey === whatsappPhoneId) {
      return c.json({
        ...
        message:
          '⚠️ ERROR: WHATSAPP_API_KEY y WHATSAPP_PHONE_ID tienen el mismo valor. Son dos credenciales DIFERENTES.',
        detail:
          'El Phone ID es un número corto (15 dígitos). El API Key es un token largo (200+ caracteres que empieza con "EAA...").',
        configSource: 'environment',
      });
    }
