...
describe.runIf(!!process.env.SUPABASE_LOCAL)(
  'WhatsApp Business API - Integración con Servidor',
  () => {
    ...
    describe('Validación de Phone Number ID', () => {
      it('debe rechazar envío con Phone Number ID inválido (+34...)', async () => {
        ...
        expect(validation.errors.some((e) => e.includes('número de teléfono'))).toBe(true);
      });
...
describe('WhatsApp Business API - Formato de Números', () => {
  const testCases = [
    ...
    {
      input: '(555) 832-7331',
      expected: '15558327331',
      description: 'Número USA sin código de país',
      defaultCode: '1',
    },
  ];
