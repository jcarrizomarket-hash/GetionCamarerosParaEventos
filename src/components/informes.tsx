  ...
  const exportarPDF = () => {
    ...
    if (tipoInforme === 'cliente' && clientAnalytics) {
      ...
      if (fechaDesde || fechaHasta)
        doc.text(`Periodo: ${fechaDesde || 'Inicio'} - ${fechaHasta || 'Fin'}`, 14, 42);
      ...
      waiterAnalytics.eventosTrabajados.forEach((e) => {
        const row = [new Date(e.fecha).toLocaleDateString(), e.cliente, e.lugar, e.horas, e.estado];
        tableRows.push(row);
      });
          ...
          <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-4">
            {tipoInforme === 'cliente' ? (
              <div>
                ...
                >
                  <option key="cliente-empty" value="">
                    -- Seleccione --
                  </option>
                  {clientes.map((c) => (
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                >
                  <option key="camarero-empty" value="">
                    -- Seleccione --
                  </option>
                  {camareros.map((c) => (
                    <option key={c.id} value={c.id}>
                      #{c.numero} - {c.nombre} {c.apellido}
                    </option>
                  ))}
                </select>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="bg-white p-6 rounded-xl border border-purple-100 shadow-sm">
              ...
              <p className="text-xs text-gray-400 mt-1">
                Promedio: {clientAnalytics.avgCamareros} / evento
              </p>
            </div>
            ...
            <div className="bg-white p-6 rounded-xl border border-green-100 shadow-sm">
              ...
              <p className="text-3xl font-bold text-gray-900">
                {clientAnalytics.porcentajePedidos}%
              </p>
              <div className="w-full bg-gray-100 rounded-full h-1.5 mt-2">
                <div
                  className="bg-green-500 h-1.5 rounded-full"
                  style={{ width: `${clientAnalytics.porcentajePedidos}%` }}
                ></div>
              </div>
            </div>
          </div>
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-200">
                {waiterAnalytics.eventosTrabajados.map((e) => (
                  <tr
                    key={`evento-${e.fecha}-${e.cliente}-${e.lugar}`}
