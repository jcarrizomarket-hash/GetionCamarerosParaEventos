...
Re-deploy the Supabase Edge Function to pick up middleware changes:

```bash
supabase functions deploy make-server-25b11ac0
```

Set the new secret in Supabase:
...
| `x-fn-secret` header | ❌ Not used | ✅ Required for mutations |
...
| CORS middleware | ❌ None | ✅ Configurable |
...
3. Remove the `VITE_SUPABASE_FN_SECRET` environment variable (not used in v1.x).
